...
inline std::string ltrim(const std::string & str)
{
    std::string s(str);
    s.erase(s.begin(),
            std::find_if_not<decltype(s.begin()), int(int)>(
...
{
    std::string s(str);
    s.erase(std::find_if_not<decltype(s.rbegin()), int(int)>(
                s.rbegin(), s.rend(), std::isspace)
                .base(),
            s.end());
    return s;
}
...
{
    const std::string buttonStrings[]
    ...
    const std::string keys[]
        = {"A",       "B",        "C",        "D",         "E",
           ...
           "Equal",   "Dash",     "Space",    "Return",    "BackSpace",
    ...
    while (std::getline(file, line)) {
        ...
        if (line[0] == '#' || line.empty())
            ...
            auto it      = std::find(std::begin(buttonStrings),
                                std::end(buttonStrings),
                                ltrim(rtrim(line.substr(0, divider)))),
                 it2     = std::find(std::begin(keys),
                                 std::end(keys),
            ...
            (state == Player1 ? p1 : p2)[i]
                = static_cast<sf::Keyboard::Key>(key);
        }
        else
            LOG(Error) << "Invalid line in key configuration at Line "
                       << line_no << std::endl;
