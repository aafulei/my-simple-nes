...
void CPU::interrupt(InterruptType type)
{
    ...
    Byte flags = f_N << 7 | f_V << 6 | 1 << 5
               |                       // unused bit, supposed to be always 1
...
void CPU::step()
{
    ...
    // first failed ExecuteImplied must be called first and ExecuteBranch must
    // be before ExecuteType0
    if (CycleLength
...
bool CPU::executeImplied(Byte opcode)
{
    switch (static_cast<OperationImplied>(opcode)) {
        ...
        case JMPI: {
            ...
            // fetched from the beginning of that page rather than the
            // beginning of the next Recreating here:
...
bool CPU::executeType1(Byte opcode)
{
    if ((opcode & InstructionModeMask) == 0x1) {
        ...
        auto    op       = static_cast<Operation1>((opcode & OperationMask)
                                          >> OperationShift);
        switch (
...
bool CPU::executeType2(Byte opcode)
{
    if ((opcode & InstructionModeMask) == 2) {
        Address location = 0;
        auto    op       = static_cast<Operation2>((opcode & OperationMask)
                                          >> OperationShift);
        auto    addr_mode
