    ...
    switch (m_pipelineState) {
            ...
            if (m_cycle > 0 && m_cycle <= ScanlineVisibleDots) {
                ...
                if (m_showSprites && (!m_hideEdgeSprites || x >= 8)) {
                    for (auto i : m_scanlineSprites) {
                        ...
                        if ((attribute & 0x40)
                            == 0) // If NOT flipping horizontally
                            x_shift ^= 7;
                        ...
                        else // 8x16 sprites
                        {
                            ...
                            addr |= (tile & 1)
                                 << 12; // Bank 0x1000 if bit-0 is high
                        }

                        sprColor |= (read(addr) >> (x_shift))
                                  & 1; // bit 0 of palette entry
                        sprColor |= ((read(addr + 8) >> (x_shift)) & 1)
                                 << 1; // bit 1
                        ...
                        break; // Exit the loop now since we've found the
                               // highest priority sprite
                    }
                }
        ...
        case PostRender:
            if (m_cycle >= ScanlineEndCycle) {
                ...
                // Should technically be done at first dot of VBlank, but this
                // is close enough
                //                     m_vblank = true;
...
{
    std::memcpy(m_spriteMemory.data() + m_spriteDataAddress,
                page_ptr,
...
Byte PPU::getData()
{
    ...
    if (m_dataAddress < 0x3f00) {
        // Return from the data buffer and store the current value in the
        // buffer
